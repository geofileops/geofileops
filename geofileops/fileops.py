...
def create_spatial_index(
    ...
    try:
        ...
        if isinstance(ex, ValueError) and str(ex).startswith(
            ...
            raise ValueError(
                f"create_spatial_index not supported for {path_info.drivername}: {path}"
            ) from ex
        else:
...
def has_spatial_index(
    ...
    try:
        if path_info.is_spatialite_based:
            ...
            raise ValueError(
                f"has_spatial_index not supported for {path_info.drivername}: {path}"
            )
...
def remove_spatial_index(
    ...
    try:
        if path_info.is_spatialite_based:
            ...
            raise ValueError(
                f"remove_spatial_index not supported for {path_info.drivername}: {path}"
            )
