...
def erase(
    ...
    # Template using aggregate function for the difference: slow for polygons with many
    # overlaps as it seems only +- 40 aggregate python (empty) calls can be ran /s.
    sql_template = f"""
        SELECT * FROM
          ( WITH layer1_diff_layer2 AS (
              SELECT layer1.rowid AS layer1_rowid
                    ,ST_GeomFromWKB(
                        ST_difference_agg(
            ...
            SELECT IIF(layer1_diff_layer2.layer1_rowid IS NOT NULL,
                       layer1.{{input1_geometrycolumn}},
                       layer1_diff_layer2.geom) AS geom
                  {{layer1_columns_prefix_alias_str}}
              FROM {{input1_databasename}}."{{input1_layer}}" layer1
              LEFT JOIN layer1_diff_layer2
                ON layer1.rowid = layer1_diff_layer2.layer1_rowid
             WHERE 1=1
               {{batch_filter}}
            LIMIT -1 OFFSET 0
          )
              ...
              SELECT layer1.rowid AS layer1_rowid
                    ,ST_GeomFromWKB(
                        ST_Difference_Collection(
                            ST_AsBinary(layer1.{{input1_geometrycolumn}}),
                            ST_AsBinary(ST_Collect(layer2.{{input2_geometrycolumn}})),
                            1)) AS geom
                FROM {{input1_databasename}}."{{input1_layer}}" layer1
                JOIN {{input1_databasename}}."{input1_layer_rtree}" layer1tree
                  ON layer1.fid = layer1tree.id
    ...
    sql_template = f"""
        SELECT * FROM
          ( WITH layer2_collect AS (
            ...
            SELECT ST_GeomFromWKB(ST_Difference_Collection(
                        ST_AsBinary(layer1.{{input1_geometrycolumn}}),
                        ST_AsBinary(layer2_collect.geom),
                        1)) AS geom
                  {{layer1_columns_prefix_alias_str}}
    ...
    sql_template = f"""
        SELECT * FROM
          ( WITH layer1_diff_layer2 AS (
            ...
            SELECT IFNULL(
                     ( SELECT geom FROM layer1_diff_layer2
                        WHERE layer1_diff_layer2.layer1_rowid = layer1.rowid
                        LIMIT -1 OFFSET 0
                     ),
                     layer1.geom
                   ) AS geom
    ...
    # - Specify return_empty=1, to have ST_Difference_Collection returns an empty
    #   geometry (not NULL) if the difference results in an empty result. With the
    #   combination of the IFNULL we know this way whether the subquery is not giving a
    #   result (no match with spatial index) or if the difference results in an empty
    #   geometry.
    sql_template = f"""
        SELECT * FROM (
          SELECT IFNULL(
