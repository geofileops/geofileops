...
def test_makevalid():
    ...
    geometrycollection_invalid = sh_geom.GeometryCollection([
            ...
            polygon_invalid, multipolygon_invalid])
...
def test_simplify_ext_invalid():
    ...
    assert isinstance(geom_simplified, sh_geom.MultiPolygon)
    assert len(geom_simplified.geoms) == 2
    assert (geometry_util.numberpoints(geom_simplified) <
            geometry_util.numberpoints(poly))
...
def test_simplify_ext_keep_points_on(tmp_path):
    ...
    grid_gdf = grid_util.create_grid(
            total_bounds=(
                    210431.875-1000, 176640.125-1000, 210431.875+1000, 176640.125+1000),
            nb_columns=2,
            nb_rows=2,
            crs='epsg:31370')
    ...
    assert (len(simplified_gdf[simplified_gdf.intersects(point_on_input_and_border)]) !=
    ...
    output_path = (
            tmp_path /
            f"simplified_lang;{tolerance_lang};{step_lang}_keep_points_on.gpkg")
