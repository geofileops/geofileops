...
def test_buffer_styles(tmp_path, suffix, epsg):
    ...
    output_path = output_path.parent / \
        f"{output_path.stem}_endcap_join{output_path.suffix}"
    gfo.buffer(
            input_path=input_path,
            output_path=output_path,
            distance=distance,
...
@pytest.mark.parametrize(
        "suffix, epsg, groupby_columns, "
        "explodecollections, expected_featurecount",
        [
...
def test_dissolve_polygons(
        tmp_path, suffix, epsg, groupby_columns,
        explodecollections, expected_featurecount):
    # Prepare test data
    input_path = test_helper.get_testfile("polygon-parcel", suffix=suffix, epsg=epsg)
    input_layerinfo = gfo.get_layerinfo(input_path)
...
def test_dissolve_polygons_aggcolumns(tmp_path, suffix):
    ...
    gfo.dissolve(
            ...
            agg_columns={"columns": [
                ...
                {"column": "lblhfdtlt", "agg": "count", "distinct": True,
                    "as": "lbl_cnt_d"},
                {"column": "lblhfdtlt", "agg": "concat", "as": "lbl_conc"},
                {"column": "lblhfdtlt", "agg": "concat", "sep": ";",
                    "as": "lbl_conc_s"},
                {"column": "lblhfdtlt", "agg": "concat", "distinct": True,
                    "as": "lbl_conc_d"},
    ...
    print("groenten.lblhfdtlt_concat_distinct: "
