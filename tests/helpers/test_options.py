...
from geofileops.helpers import options
...
def test_copy_layer_sqlite_direct() -> None:
    ...
    if key in options.os.environ:
        del options.os.environ[key]

    # Test setting the option permanently
    options.copy_layer_sqlite_direct(False)
    assert os.environ[key] == "FALSE"

    # Test setting the option temporarily using context manager
    with options.copy_layer_sqlite_direct(True):
