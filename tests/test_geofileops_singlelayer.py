...
@pytest.mark.parametrize(
        ...
        get_combinations_to_test(
                ["geofileops.geoops", "geofileops.util._geoops_gpd"]))
def test_buffer(
        tmp_path, suffix, epsg, fileops_module, testfile):
    ...
    assert_geodataframe_equal(
            output_gdf, expected_gdf, promote_to_multi=True,
            check_less_precise=check_less_precise, sort_values=True)
...
def test_buffer_negative(
        tmp_path, suffix, fileops_module, testfile):
    """ Buffer basics are available both in the gpd and sql implementations.
    """
...
@pytest.mark.parametrize(
        "fileops_module", ["geofileops.geoops", "geofileops.util._geoops_gpd"])
def test_buffer_negative_explode(
        tmp_path, fileops_module):
    """ Buffer basics are available both in the gpd and sql implementations.
    """
    input_path = test_helper.get_testfile("polygon-parcel")
    ...
    output_path = output_path.parent / \
        f"{output_path.stem}_m10m_explode{output_path.suffix}"
    distance = -10
    geoops.buffer(
            input_path=input_path,
            output_path=output_path,
